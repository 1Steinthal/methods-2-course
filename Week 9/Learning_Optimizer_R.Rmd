...
We know that one can find the local minimum by finding the x value in which f'(x)=0

```{r}
f<-expression(10*x^2+2*x+30) #f'(x)=20*x+2 , f''(x)=20
f_prime<-D(f,'x')

solve(f_prime,)

solve(polynomial,c(0))
...
Take the double derivative to see whether it is positive or negative. (negative means max, positive means minimum)
...
#plot the function as a line (type = "l"). 
plot(x_plot, f(x_plot), type = "l")
```
  
  b) Find the 4 minimums using the optim() function. 
```{r}
optim(1, fn = f, method= "Brent", lower = -10, upper = 10)
x = -10
repeat{
  print(paste("for x = ", x," the closest minimum is at x =",optim(x, f, method = "CG")$par))
  x = x +1
  if (x > 10) break("X limit reached")
}
```
...
repeat{
  print(paste("for x = ", x," the closest maximum is at x =",optim(x, fungi, method = "CG")$par))
  x = x +1
  if (x > 10) break("X limit reached")
}
```
...
X_matrix <- cbind(rep(1, 5), x)      # adding a column of 1s as intercept to the design matrix X (1e2 x 1)
theta <- rep(0,2)               # set up the initial theta 2 x 1 vector
