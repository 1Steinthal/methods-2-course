...
```
The two functions represent draws from a binomial distribution. The first one samples 10 draws 2e4 times and returns the hit count for each draw, thus ending up with 2e4 hitcount. The dbinom does NOT sample but returns the probability distributions for x numbers of hits in a 10-draw sample. It thus calculates and does not sample. Because of the probability being 1/6 it makes sense, that the highest probability of x hits is x=2 (10/6 ~ 2) 
...
## 1 - The point estimate difference
diff_est<-abs(a_mean-b_mean)
## 2 - The uncertanty as summed standard deviation
uncer_est<-sqrt(a_sd^2+b_sd^2)
...
sample(c("friday","saturday","sunday"), size = 20, replace = TRUE, prob = c(0.8,0.7,0.99))
...
There is an error in the code. It returns only the last if-statement (if it meets the conditions) instead of comparing all the values. There should be a statement to only paste the new sum if the exceeds 
...
(c) Using your simulations, make a scatterplot of the number of shots the player will take and the
...
samp_dist_of_samp_means_basket <- c()
for (i in 1:1000){
  samp_dist_of_samp_means_basket[i] <- mean(basketball())
}
hist(samp_dist_of_samp_means_basket)


x3 <- dbinom(0:30, size = 30 , prob = 0.6)
plot(x = 0:30, y = x3, type = "l")
